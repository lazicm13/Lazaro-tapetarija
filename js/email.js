...
function sendRequest() {
	...
	jQuery('#contactWrapper').append('<div class="success"><h4>Email uspesno poslat!</h4><br><p>Hvala sto koristite kontakt formu <strong>'+decodeURIComponent(name)+'</strong>! Email poslat! Uskoro cemo Vas kontaktirati!</p></div>');
	}
}
function sendRequest_booking() {
	...
	jQuery('#contactWrapper').append('<div class="success"><h4>Email poslat!</h4><br><p>Hvala sto koristite kontakt formu<strong>'+decodeURIComponent(name)+'</strong>! Email uspesno poslat, uskoro cete biti kontaktirani.</p></div>');
	}
}
...
function check_values() {
	...
	var errors=0;
	 ...
	 if($j('#contactform #name').val()=='') {
	 	...
	 	if(!hasClass)
	 	    $j('#contactform #name').parent().append('<label for="contactname" generated="true" class="error">Unesite ime</label>');
		...
		var hasClass=$j('#contactform #email').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #email').parent().append('<label for="contactname" generated="true" class="error">Molim Vas unesite validnu email adresu</label>');	
		...
		var hasClass=$j('#contactform #subject').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Morate napisati temu!</label>');	
		...
		var hasClass=$j('#contactform #body').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #body').parent().append('<label for="contactname" generated="true" class="error">Morate napisati poruku!</label>');	
	...
	if(errors==0) {
			...
			document.getElementById("submit").value='Molim sacekajte..';
			sendRequest();
	}
}
